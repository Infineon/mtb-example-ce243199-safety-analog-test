# Safety Test Library (STL) : ADC

This code example demonstrates the usage of ModusToolbox&trade; Safety Test Library (MTB STL) to test the ADC of the MCU.
...
[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce243199-safety-analog-test)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
- [XMC5200 Evaluation Kit](https://www.infineon.com/evaluation-board/KIT-XMC52-EVK) (`KIT-XMC52-EVK`)
...
The following example clones the "[ADC test](https://github.com/Infineon/mtb-example-ce243199-safety-analog-test)" application with the desired name "ADC_Test" configured for the *KIT_XMC52_EVK* BSP into the specified working directory *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC52_EVK --app-id mtb-example-ce243199-safety-analog-test --user-app-name ADC_Test --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that **MTB STL: ADC Safety Test** is displayed on the UART terminal.
...
6. Adjust the onboard potentiometer VR1 as needed.
...
This example uses the SAR ADC peripheral on the MCU to perform a safety self-test on the analog measurement chain. The design.modus file configures two channels: channel 0 is routed to the on-board potentiometer (CYBSP_POT, P6.0) and channel 1 is routed to the internal bandgap voltage reference (VBG, ~900 mV).
The example begins by initializing the BSP configuration and setting up retarget-io for debug prints. The ADC is initialized using the PDL driver and the EPASS reference buffer is enabled, which is mandatory for reading internal reference channels. A self-test is then run using the SelfTests_ADC() API from the mtb-stl (https://github.com/Infineon/mtb-stl) safety library.
The library normalizes the potentiometer reading (channel 0) against the VBG reading (channel 1) using the formula: adc_res = (ch0_counts × 900) / ch1_counts, converting the result to millivolts. The test passes when the normalized result is within the allowed accuracy window of 900 mV. Before running the example, adjust the on-board potentiometer until its output reaches approximately 900 mV. The PASS or FAIL result is printed to the debug UART console, and the device halts after a single run.
...
Document title: *CE243199* – *Safety Test Library (STL) : ADC Test*
